...
export const FormRegisterAnnouncement = ({ setOpen }: iFormRegisterAnnouncement) => {
    const { getCars, models, getModels, brands, postAnnouncement } = useContext(UserContext);
    ...
    const submitForm = (data: iFormAnnouncement) => {
        const imageGallery = getValues("image_gallery")?.filter((elem) => elem.image !== "") || [];

        setValue("image_gallery", imageGallery);

        postAnnouncement(data).then((status) => (status === 201 ? setOpen(false) : null));
    };
